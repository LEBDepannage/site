...
export function TestimonialsPreview({ data }: TestimonialsPreviewProps) {
  return (
    <section className="py-20 max-lg:landscape:py-12 md:py-28 bg-white">
      <div className="container mx-auto px-4 md:px-8 lg:px-12">
        ...
        {/* Stats cards */}
        <div className="grid gap-6 max-lg:landscape:gap-4 md:grid-cols-3 mb-16 max-lg:landscape:mb-8 max-w-5xl mx-auto">
          {data.stats.map((stat) => {
            const IconComponent = (LucideIcons as any)[stat.icon] || ThumbsUp
            return (
              <Card key={stat._key} className="bg-gradient-to-br from-slate-50 to-white border-2 border-border hover:border-primary text-center py-10 max-lg:landscape:py-6 shadow-sm hover:shadow-lg transition-all">
                <CardContent className="p-0">
                  <IconComponent className="h-10 w-10 max-lg:landscape:h-8 max-lg:landscape:w-8 text-primary mx-auto mb-4 max-lg:landscape:mb-2" />
                  <p className="text-5xl max-lg:landscape:text-3xl font-bold text-foreground mb-2 max-lg:landscape:mb-1">{stat.value}</p>
                  <p className="text-muted-foreground max-lg:landscape:text-sm font-medium">{stat.label}</p>
                </CardContent>
              </Card>
            )
          })}
        ...
        <div className="max-w-4xl mx-auto">
          <p className="text-center text-lg max-lg:landscape:text-base text-muted-foreground mb-8 max-lg:landscape:mb-4 font-medium">
